...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (track && prevBtn && nextBtn && viewport) {
    ...
    // We want to clone cards to achieve infinite loop.
    // Desktop shows 3 cards, so cloning 3 at each end is perfect.
    const cloneCount = 3;

    // Clone first 3 cards and append them
    for (let i = 0; i < cloneCount; i++) {
      const clone = originalCards[i].cloneNode(true);
      // Remove original ID to prevent duplicate IDs in document
      clone.removeAttribute('id');
      track.appendChild(clone);
    }

    // Clone last 3 cards and prepend them
    ...
    let currentIndex = cloneCount; // Start at the first original card (index 3)
    let isTransitioning = false;
    let autoplayInterval = null;

    // Update track position based on active card offset
    function updateCarousel(instant = false) {
      ...
      // Calculate offset dynamically using offsetLeft of the target card
    ...
    track.addEventListener('transitionend', () => {
      ...
      // If we reached the end clones (index >= originalCount + prependedClones)
      if (currentIndex >= originalCount + cloneCount) {
        currentIndex = cloneCount; // Wrap back to first original card
        updateCarousel(true);
      }
      // If we reached the beginning clones (index < prependedClones)
      else if (currentIndex < cloneCount) {
        currentIndex = originalCount + currentIndex; // Wrap to corresponding original card
        updateCarousel(true);
      }
    });
    ...
    // We delay slightly or wait next frame to make sure offsets are calculated correctly by browser
    ...
    // Handle window resizing to keep the correct position
